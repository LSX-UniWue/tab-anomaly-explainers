...
invert_dict_erp = {'captum_grad_input_zeros': False,
                   'captum_gradient_zeros': True,
                   'captum_intgrad_mean': True,
                   'captum_intgrad_NN': True,
                   'captum_intgrad_single_optimum': True,
                   'captum_intgrad_zeros': False,
                   'captum_lrp_grad': False,
                   'captum_lrp_nograd': True,
                   'lime_kmeans': False,
                   'shap_kmeans': False,
                   'shap_mean': False,
                   'shap_NN': False,
                   'shap_single_optimum': False,
                   'shap_zeros': False}


def get_mean_scores():
    base_folder = Path('outputs/explanation/erp_fraud/rashomon')
    ...
    for in_folder in base_folder.iterdir():
        ...
        best_seeds = [19, 25, 27, 32, 38, 51, 5, 71, 74, 80, 83]
        small_outs = [x for x in outs if x['seed'] in best_seeds]
        assert len(small_outs) == len(best_seeds)
        small_out_dict = {'type': 'small',
                          'Model': in_folder.name,
                          'gte 0.5': len([1 for x in small_outs if x['roc_mean'] >= 0.5]),
                          'lt 0.5': len([1 for x in small_outs if x['roc_mean'] < 0.5]),
                          'Mean mean ROCs': np.mean([x['roc_mean'] for x in small_outs]),
                          'Std mean ROCs': np.std([x['roc_mean'] for x in small_outs]),
                          'Mean mean COSs': np.mean([x['cos_mean'] for x in small_outs]),
                          'Std mean COSs': np.std([x['cos_mean'] for x in small_outs])}
        print(large_out_dict)
        print(small_out_dict)
        out_file = in_folder.parent / 'summary.csv'
        if out_file.exists():
            summary = pd.read_csv(out_file, header=0)
            summary = summary.append(large_out_dict, ignore_index=True)
            summary = summary.append(small_out_dict, ignore_index=True)
        else:
            summary = pd.DataFrame(large_out_dict, index=[0])
            summary = summary.append(small_out_dict, ignore_index=True)
        summary.to_csv(out_file, index=False)


def hist_plots(in_folder, _k=5, type='large', just_legend=False):
    if just_legend:
    ...
    for expl_file in in_folder.glob('*.csv'):
        ...
        if flip_scores:
            expl = -1 * expl
        expl = xai_to_categorical(expl_df=expl, dataset=data)
...
if __name__ == '__main__':
    ...
    score_path = Path('outputs/explanation/erp_fraud/rashomon')
